...
//   async generateIBCycleActivities(tags, selections, subject = 'history', numLessons = null) {
//     try {
//       const anthropicClient = await initAnthropic();
//       const subjectPrompt = this.SUBJECT_PROMPTS[subject] || this.SUBJECT_PROMPTS.history;
...
//       const response = await anthropicClient.messages.create({
//         model: process.env.CLAUDE_MODEL || 'claude-sonnet-4-20250514',
//         max_tokens: numLessons ? numLessons * 2000 : 8000, // Scale tokens based on number of lessons
//         system: `${subjectPrompt}
...
//           {
//             role: 'user',
//             content: `${numLessons ? `Create a comprehensive IB History DP unit plan consisting of ${numLessons} lessons, using the flipped learning model. You MUST use ONLY the selected activities provided and incorporate any uploaded file content. Each lesson should be clearly delineated.` : `Create a comprehensive IB History DP lesson plan using the flipped learning model. You MUST use ONLY the selected activities provided and incorporate any uploaded file content.`}
